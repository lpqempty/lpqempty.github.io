...
window.addEventListener('scroll', adjustBlockHeight);

function adjustBlockHeight() {
  const main = document.querySelector(".main");
  const body = document.querySelector("body");
  const maxHeightbody = body.offsetHeight;
  const maxHeight = main.offsetHeight;
  let scrollHeight = window.scrollY;
  let block = document.querySelector(".header");
  let newHeight = window.innerHeight + scrollHeight;
...
function downloadTableCheck() {
  ...
  rows.forEach((row) => {
    ...
    if (!rowData.every(element => element === '')){
      check = check + 1;
    };
  });
  if (check != 0){
    ...
    alert("Нельзя скачать пустую таблицу!")
  };
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document
    ...
    .addEventListener("click", downloadTableCheck);
